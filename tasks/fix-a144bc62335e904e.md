# fix-a144bc62335e904e — Duplicate E4T1 test files (start.json + E4T1.json)

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 UCB

Both `tests/specs/start.json` and `tests/specs/E4T1.json` contain identical `/start` dialog specs. Similarly, `tests/commands/start.json` and `tests/commands/E4T1.json` both declare `["/start"]`. The CI gate globs and merges all files in these directories, so the same spec runs twice redundantly. One pair should be removed to avoid dead test definitions.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-a144bc62335e904e.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-a144bc62335e904e.json` (and any new command `tests/commands/fix-a144bc62335e904e.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-a144bc62335e904e.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-a144bc62335e904e.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.

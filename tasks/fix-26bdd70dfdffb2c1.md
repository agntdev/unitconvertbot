# fix-26bdd70dfdffb2c1 — spec.md says TELEGRAM_TOKEN but code reads BOT_TOKEN

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 UCB

`docs/spec.md` (line 27, 100) documents the required env var as `TELEGRAM_TOKEN`, but `src/index.ts` (line 4) reads `process.env.BOT_TOKEN` and `.env.example` (line 1) declares `BOT_TOKEN=`. The code and `.env.example` are internally consistent; the spec document is out of date. This could confuse developers following the spec for deployment.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-26bdd70dfdffb2c1.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-26bdd70dfdffb2c1.json` (and any new command `tests/commands/fix-26bdd70dfdffb2c1.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-26bdd70dfdffb2c1.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-26bdd70dfdffb2c1.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.

# fix-d5b5a6378bc54a84 — T02 specs expect inline-keyboard /start + menu:ping callback; no handler provides it

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 UCB

`tests/specs/T02.json` expects `/start` to reply with `reply_markup: { inline_keyboard: [[{ text: "Ping", callback_data: "menu:ping" }]] }` and a second step sends `callback: "menu:ping"` expecting `answerCallbackQuery` + `sendMessage("pong")`. `tests/commands/T02.json` declares `/start`. No handler implements this: `src/handlers/start.ts` sends only plain welcome text without any inline keyboard, and no handler anywhere registers a `menu:ping` callback handler. These are orphaned specs/commands with no matching implementation.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-d5b5a6378bc54a84.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-d5b5a6378bc54a84.json` (and any new command `tests/commands/fix-d5b5a6378bc54a84.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-d5b5a6378bc54a84.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-d5b5a6378bc54a84.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.

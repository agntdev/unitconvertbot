# fix-f111901666a307cc — milesToKilometers uses imprecise reciprocal division instead of direct conversion

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 UCB

In `src/utils/conversions.ts:14`, `milesToKilometers` computes `miles / 0.621371`. The exact conversion standard is 1 mile = 1.609344 km. Using division by the approximate reciprocal 0.621371 (whose true reciprocal is ~1.609347) introduces systematic imprecision vs. the correct `miles * 1.609344`. The 2-decimal rounding masks this for common inputs, but the underlying value is mathematically wrong.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-f111901666a307cc.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-f111901666a307cc.json` (and any new command `tests/commands/fix-f111901666a307cc.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-f111901666a307cc.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-f111901666a307cc.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.

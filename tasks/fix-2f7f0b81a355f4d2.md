# fix-2f7f0b81a355f4d2 — Missing handler module src/handlers/E2T1.ts

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 UCB

The task spec requires a handler in `src/handlers/E2T1.ts` that default-exports a grammY `Composer` wiring `parseNumber` into the bot. This file does not exist. `parseNumber` is never imported or used anywhere in the codebase — the utility is dead code and the feature has no bot-command surface. Per the Implementation Contract: "the feature's command/handler must be registered via its default-exported `Composer` in `src/handlers/<slug>.ts` (auto-loaded) and reachable from the bot's command surface."

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-2f7f0b81a355f4d2.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-2f7f0b81a355f4d2.json` (and any new command `tests/commands/fix-2f7f0b81a355f4d2.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-2f7f0b81a355f4d2.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-2f7f0b81a355f4d2.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.

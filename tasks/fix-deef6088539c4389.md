# fix-deef6088539c4389 — E4T1 /start command list omits /km2mi

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 UCB

`src/handlers/start.ts:8-14` lists available commands but omits `/km2mi`, which was added in E1T3 (`src/handlers/E1T3.ts`). The `/help` handler (E4T2) correctly includes `/km2mi`. Users who see `/start` will not discover the km2mi converter. The list is incomplete and inconsistent with the actual command surface.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-deef6088539c4389.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-deef6088539c4389.json` (and any new command `tests/commands/fix-deef6088539c4389.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-deef6088539c4389.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-deef6088539c4389.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.

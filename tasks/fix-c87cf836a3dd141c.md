# fix-c87cf836a3dd141c — No test for error-catching behavior — only happy path is tested

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 UCB

The test spec `tests/specs/E3T1.json` contains only one test case: `/m2k 5` → `"8.05"`, which verifies that the error middleware does not break normal operation. There is **no test case** that triggers an actual handler error and asserts the response `"An error occurred; please try again"`. The core error-catching behavior is completely untested — if the middleware were accidentally removed or broken, no test would fail.

A proper test should include a step that causes a handler to throw (e.g., a command that throws unexpectedly) and expects `sendMessage` with `text: "An error occurred; please try again"`.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-c87cf836a3dd141c.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-c87cf836a3dd141c.json` (and any new command `tests/commands/fix-c87cf836a3dd141c.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-c87cf836a3dd141c.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-c87cf836a3dd141c.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
